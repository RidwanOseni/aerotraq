...
import { storyClient } from '../../../lib/storyClient'; // Import the configured Story Protocol client
import { toHex, zeroAddress, Address, Hex } from 'viem'; // Needed for hex conversions and addresses
import { LicenseTerms } from '@story-protocol/core-sdk'; // Needed for LicenseTerms type
import { spawn } from 'child_process'; // To call the Python script
...
export async function POST(request: NextRequest) {
    try {
        ...
        // **1. Define/Register License Terms** [18]
        // Using example Commercial Remix PIL terms. Adjust parameters as needed for your project.
        const WIP_TOKEN_ADDRESS = "0x1514000000000000000000000000000000000000" as Address; // Example $WIP address from Story Protocol docs [19]
        const ROYALTY_POLICY_LAP_ADDRESS = "0xBe54FB168b3c982b7AaE60dB6CF75Bd8447b390E" as Address; // Example RoyaltyPolicyLAP address from Story Protocol docs [19]

        const commercialRemixTerms: LicenseTerms = {
            transferable: true,
            royaltyPolicy: ROYALTY_POLICY_LAP_ADDRESS,
            defaultMintingFee: 0n, // Example: Free to mint licenses [19]
            expiration: 0n, // Example: No expiration [20]
            commercialUse: true, // Allowed for commercial use [20]
            commercialAttribution: true, // Attribution required for commercial use [20]
            commercializerChecker: zeroAddress, // No specific checker contract
            commercializerCheckerData: '0x' as Hex,
            commercialRevShare: 10, // Example: 10% revenue share to licensor [20]
            commercialRevCeiling: 0n, // No revenue ceiling
            derivativesAllowed: true, // Derivatives are allowed [20]
            ...
            currency: WIP_TOKEN_ADDRESS, // Currency for fees/revenue share [21]
            uri: "", // Optional: URI for license terms metadata
        };

        // Register the license terms on Story Protocol.
        // If these exact terms were already registered, the protocol would return the existing ID [21].
        ...
        // **2. Mint IP Asset and Attach License Terms** [22]
        // This combines minting an SPG NFT, registering it as an IP Asset, attaching metadata,
        // and attaching license terms in one call [22, 23].
        ...
        // The IP metadata should link back to the DGIP data on IPFS [24].
        ...
        const ipMetadata = {
            ipMetadataURI: ipfsGatewayUrl, // Link to IPFS if CID available [25]
            ipMetadataHash: dgipDataHash as Hex, // Use the hash of the DGIP data [25]
        ...
        const mintAndRegisterResponse = await storyClient.ipAsset.mintAndRegisterIpAssetWithPilTerms({
            ...
            licenseTermsData: [{ terms: commercialRemixTerms }], // Attach the terms we just defined/registered [26]
            ipMetadata: ipMetadata, // Attach the metadata linking to DGIP [26]
            allowDuplicates: true, // Set based on your requirement [23, 27]
        ...
        // **3. Store Story Protocol Details in Backend Storage** [28]
