...
import { ComplianceSuggestions } from "@/components/compliance-suggestions"; // Imports ComplianceSuggestionsProps from here [compliance-suggestions-tsx.txt, 161]
...
// Import DgipSimulationDisplayProps and DgipLogEntry from the shared types file [new-error.txt, 327]
import { DgipSimulationDisplayProps, DgipLogEntry } from '@/lib/types'; 

// Dynamically import DgipSimulationDisplay with no SSR
const DgipSimulationDisplay = dynamic<DgipSimulationDisplayProps>( // <-- Add this generic type here
    () => import('@/components/dgip-simulation-display'),
    {
      ssr: false,
      loading: () => (
        <>Loading DGIP data...</>
...
export default function RegisterFlightPage() {
    ...
    // New state to force remount of DGIP simulation display
    ...
    useEffect(() => {
        let intervalId: NodeJS.Timeout | undefined; // FIX: Added 'undefined' to the type [new-error.txt, 342]
        if (isSimulating && simulatedDgip.length > 0) {
            ...
            intervalId = setInterval(() => {
                setCurrentLogIndex((prevIndex) => {
                    if (prevIndex < simulatedDgip.length - 1) {
                        ...
                        // Ensure the last log entry is displayed upon simulation completion
                        const lastLogEntry = simulatedDgip[simulatedDgip.length - 1];
                        if (displayLog !== lastLogEntry) { // FIX: Only update if value changes [new-error.txt, 343]
                            setDisplayLog(lastLogEntry);
                        }
            ...
            // If simulation is not active but data exists (e.g., after simulation finishes or on page load with existing data)
            // Ensure displayLog shows the last entry
            const lastLogEntry = simulatedDgip[simulatedDgip.length - 1];
            if (displayLog !== lastLogEntry) { // FIX: Only update if value changes [new-error.txt, 344]
                setDisplayLog(lastLogEntry);
            }
        } else {
            // If simulation data is empty or reset, ensure display is cleared
            if (displayLog !== null) { // FIX: Only update if value changes (e.g., from an old state to null) [new-error.txt, 345]
                setDisplayLog(null);
            }
        }

        // Update the displayed log entry whenever currentLogIndex or simulatedDgip changes
        // This runs for active simulation or immediately if currentLogIndex changes without interval
        if (simulatedDgip.length > 0 && currentLogIndex < simulatedDgip.length) {
            const currentLogEntry = simulatedDgip[currentLogIndex];
            if (displayLog !== currentLogEntry) { // FIX: Only update if value changes [new-error.txt, 345]
                setDisplayLog(currentLogEntry);
            }
        }
    ...
    }, [isSimulating, simulatedDgip, currentLogIndex, displayLog]); // Added displayLog to dependencies for accurate comparison [new-error.txt, 346]
    ...
    async function onSubmit(values: FlightFormData) {
        ...
        setDisplayLog(null);
        setSimulationKey(prevKey => prevKey + 1); // Increment key to force remount on new submission
        ...
        try {
            ...
            if (validationData.result && Array.isArray(validationData.result.complianceMessages)) {
                ...
                try {
                    ...
                    const client = getPublicClient(config); // Pass the config to getPublicClient [new-error.txt, 353]
                    if (!client) {
    ...
    const handleStartSimulation = async () => {
        ...
        setSimulationKey(prevKey => prevKey + 1); // Increment key to force remount on simulation start
    ...
    const handleProcessAndUploadDgip = async () => {
        ...
        try {
            ...
            try {
                ...
                // Ensure newDgipDataHash has 0x prefix and correct length (already checked above, but re-checked for safety before the contract call)
    ...
    return (
        <main className="flex min-h-screen flex-col items-center justify-between p-24">
            <Card className="w-full max-w-2xl mx-auto">
                <CardHeader>
                    <CardTitle className="text-3xl font-bold">Register Flight</CardTitle>
                ...
                <CardContent className="space-y-6">
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            {showSuggestions && validationResultData?.complianceMessages && validationResultData.complianceMessages.length > 0 && (
                                <ComplianceSuggestions messages={validationResultData.complianceMessages} />
                            )}
                    ...
                    {/* Separator between initial registration and DGIP steps */}
                    {validationResultData?.is_critically_compliant && validationResultData.dataHash && (
                    ...
                    {showRegistrationDetails && (
                        <Card className="w-full mt-6">
                            <CardHeader>
                                ...
                                <CardDescription>On-chain and IPFS data for this flight.</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-2">
                                {/* Display Initial Registration Details */}
                                {validationResultData?.dataHash && (
                                    <p className="text-sm">
                                        <span className="font-semibold">*Initial Flight Plan Hash (Keccak256):*</span>{" "}
                                        <span className="font-normal break-all">{validationResultData.dataHash}</span>
                                    </p>
                                )}
                                ...
                                {validationResultData?.ipfsCid && (
                                    <p className="text-sm">
                                        <span className="font-semibold">*Initial Flight Plan IPFS CID:*</span>{" "}
                                        <a
                                            href={`https://ipfs.io/ipfs/${validationResultData.ipfsCid}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="text-blue-500 hover:underline font-normal break-all"
                                        >
                                            {validationResultData.ipfsCid}
                                        </a>
                                    </p>
                                ...
                                {dgipDataHash && (
                                    <p className="text-sm">
                                        <span className="font-semibold">*DGIP Log Data Hash (Keccak256):*</span>{" "}
                                        <span className="font-normal break-all">{dgipDataHash}</span>
                                    </p>
                                )}
                                {dgipIpfsCid && (
                                    <p className="text-sm">
                                        <span className="font-semibold">*DGIP Log IPFS CID:*</span>{" "}
                                        <a
                                            href={`https://ipfs.io/ipfs/${dgipIpfsCid}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="text-blue-500 hover:underline font-normal break-all"
                                        >
                                ...
                                {dgipRegistrationTxHash && (
                                    <p className="text-sm">
                                        <span className="font-semibold">*DGIP On-Chain Linking Tx Hash:*</span>{" "}
                                        <a
                                ...
                                {storyProtocolResult && (
                                    <>
                                        ...
                                        <h3 className="text-md font-semibold mt-4">Story Protocol IP Asset Details:</h3>
                                        <p className="text-sm">
                                            <span className="font-semibold">*IP Asset ID:*</span>{" "}
                                            <a
                                                href={`https://aeneid.explorer.story.foundation/ip/${storyProtocolResult.ipId}`}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="text-blue-500 hover:underline font-normal break-all"
                                            >
                                                {storyProtocolResult.ipId}
                                            </a>
                                        </p>
                                        <p className="text-sm">
                                            <span className="font-semibold">*License Terms ID:*</span> {storyProtocolResult.licenseTermsId}
                                        </p>
                                        <p className="text-sm">
                                            <span className="font-semibold">*Minted NFT Token ID:*</span> {storyProtocolResult.mintedTokenId}
                                        </p>
                                        {storyProtocolResult.dbUpdateMessage && (
                                            <p className="text-sm">
                                                <span className="font-semibold">*Database Update Status:*</span>{" "}
                                                <span className="font-normal break-all">{storyProtocolResult.dbUpdateMessage}</span>
                                            </p>
                                        )}
                                    </>
                                )}
